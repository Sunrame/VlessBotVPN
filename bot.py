...
# --- БАЗА ДАННЫХ (SQLite сохраняет данные между перезагрузками) ---
def init_db():
...
def activate_user_in_db(user_id, active=1, custom_expiry=None, add_months=1):
    ...
    if custom_expiry:
        ...
        # Если подписка активна — прибавляем к ней, если нет — от текущего момента
...
def sync_with_panel(user_id, username):
    """Синхронизирует состояние базы данных с реальностью в панели"""
    ...
    try:
        ...
        client_stats = obj['clientStats']
        
        c_set = next((c for c in clients if c['email'] == email), None)
        c_stat = next((c for c in client_stats if c['email'] == email), None)
        ...
        if c_set and c_set.get('enable'):
            ...
            if exp > time.time() or exp == 0:
                ...
                return {"used": c_stat.get('up', 0) + c_stat.get('down', 0), "limit": c_set.get('totalGB', 0), "expiry": exp}
...
def main_markup():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📜 Оферта", callback_data="rules")]
    ])
...
async def cmd_start(message: types.Message, command: CommandObject):
    ...
    # Синхронизация при заходе
    await asyncio.get_event_loop().run_in_executor(None, sync_with_panel, message.from_user.id, message.from_user.username)
    
    await message.answer(
        f"👋 Привет, {hbold(message.from_user.full_name)}!\n\nИспользуя бота, вы соглашаетесь с правилами (пункт Оферта).", 
        reply_markup=main_markup(), parse_mode="HTML"
    )

@router.callback_query(F.data == "profile")
async def show_profile(callback: CallbackQuery):
    st = await asyncio.get_event_loop().run_in_executor(None, sync_with_panel, callback.from_user.id, callback.from_user.username)
    ...
    u_gb = round(st['used']/(1024**3), 2) if st else "0"
    l_gb = round(st['limit']/(1024**3), 2) if st else "50"
    
    await callback.message.edit_text(
        f"👤 <b>Личный кабинет</b>\n\n⏳ Осталось: <b>{max(0, int(days))} дн.</b>\n📊 Трафик: <b>{u_gb}/{l_gb} ГБ</b>\n📱 Лимит: 3 устройства", 
        reply_markup=main_markup(), parse_mode="HTML"
    )
...
async def show_ref(callback: CallbackQuery):
    ...
    text = (f"🤝 <b>Партнерка</b>\n\nПригласи 5 друзей и получи <b>+1 месяц</b> бесплатно!\n"
            f"Бонус суммируется с текущей подпиской.\n\n"
            f"📈 Прогресс: <b>{count}/5</b>\n"
            f"🔗 Ссылка: {hcode(f'https://t.me/{me.username}?start={callback.from_user.id}')}")
    
    markup = []
    if count >= 5: markup.append([InlineKeyboardButton(text="🎁 Забрать месяц", callback_data="claim_reward")])
...
async def claim_reward(callback: CallbackQuery):
    ...
    if cursor.fetchone()[0] >= 5:
        ...
        await callback.answer("✅ Бонусный месяц добавлен!", show_alert=True)
        await show_ref(callback)
    else: conn.close()

@router.callback_query(F.data == "rules")
async def show_rules(callback: CallbackQuery):
    text = ("📜 <b>Оферта и правила:</b>\n\n"
            "1. Бот сохраняет Telegram ID для работы подписки.\n"
            "2. Запрещено использовать VPN для нарушения законов РФ.\n"
            "3. Возврат средств не предусмотрен.\n"
            "4. 1 аккаунт = 3 устройства.")
...
async def show_guide(callback: CallbackQuery):
    await callback.message.edit_text("📖 <b>Инструкция Happ:</b>\n1. Скачай <b>Happ</b>.\n2. Копируй ссылку из бота.\n3. Импортируй через буфер.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="to_main")]]), parse_mode="HTML")
...
async def user_paid(callback: CallbackQuery):
    await callback.message.answer("⏳ Заявка у админов.")
...
async def adm_ap(callback: CallbackQuery):
    ...
    if lnk:
        ...
        if ref_id:
            ...
            conn.commit()
            try: await bot.send_message(ref_id, "🔔 Друг купил подписку! Прогресс +1.")
            except: pass
        conn.close()
