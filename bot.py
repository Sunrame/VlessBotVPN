...
def activate_user_in_db(user_id, active=1, months=1):
    conn = sqlite3.connect('users.db'); cursor = conn.cursor()
    cursor.execute('SELECT expiry_date, is_active FROM users WHERE user_id = ?', (user_id,))
    row = cursor.fetchone()
    now = int(time.time())
    added_time = months * 30 * 24 * 60 * 60
    if row and row[1] == 1 and row[0] > now:
        new_expiry = row[0] + added_time
    else:
        new_expiry = now + added_time
    cursor.execute('UPDATE users SET is_active = ?, expiry_date = ? WHERE user_id = ?', (active, new_expiry, user_id))
...
def main_markup():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="📜 Оферта и правила", callback_data="rules")]
    ])
...
async def cmd_start(message: types.Message, command: CommandObject):
    ...
    await message.answer(
        ...
        "Используя этого бота, вы автоматически соглашаетесь с правилами сервиса и обработкой персональных данных.", 
...
async def show_rules(callback: CallbackQuery):
    rules_text = (
        ...
        "1. <b>Сервис</b> предоставляет услуги доступа к частной сети (VPN) в ознакомительных целях.\n"
        "2. <b>Персональные данные:</b> Бот сохраняет ваш Telegram ID и Username для идентификации доступа. Данные не передаются третьим лицам.\n"
        "3. <b>Ответственность:</b> Пользователь несет полную ответственность за свои действия в сети Интернет при использовании сервиса. Запрещено использование сервиса для совершения противоправных действий согласно законодательству РФ.\n"
        "4. <b>Возврат:</b> Цифровые товары (подписки) возврату не подлежат после активации доступа.\n"
        "5. <b>Ограничения:</b> 1 аккаунт = до 3 устройств одновременно."
    )
...
async def show_ref(callback: CallbackQuery):
    d = get_user_db_data(callback.from_user.id)
    friends_count = d[1] if d else 0
    me = await bot.get_me()
    text = (f"🤝 <b>Партнерка</b>\n\nПригласи 5 друзей и получи <b>+1 месяц</b>!\n\n"
            f"📈 Прогресс: <b>{friends_count}/5</b>\n"
            f"🔗 Ссылка: {hcode(f'https://t.me/{me.username}?start={callback.from_user.id}')}")
    markup = []
    if friends_count >= 5: markup.append([InlineKeyboardButton(text="🎁 Забрать бонус", callback_data="claim_reward")])
...
async def claim_reward(callback: CallbackQuery):
    ...
    if count >= 5:
        ...
        conn.commit(); conn.close()
        activate_user_in_db(callback.from_user.id, active=1)
...
async def to_main(callback: CallbackQuery): await callback.message.edit_text("Выбери действие:", reply_markup=main_markup())
...
async def adm_ap(callback: CallbackQuery):
    ...
    if lnk:
        ...
        if ref_id:
            ...
            conn.commit()
            try: await bot.send_message(ref_id, "🔔 Друг купил подписку! +1 к прогрессу.")
            except: pass
        conn.close()
...
async def admin_take(message: types.Message, command: CommandObject):
    ...
    c.execute('SELECT user_id, username FROM users WHERE user_id = ? OR username = ?', (t, t))
    r = c.fetchone(); conn.close()
    if r:
        await asyncio.get_event_loop().run_in_executor(None, delete_vpn_client, r[0], r[1])
        activate_user_in_db(r[0], active=0, months=0)
