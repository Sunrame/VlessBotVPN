...
async def show_profile(callback: CallbackQuery):
    ...
    # Проверка на наличие данных и на активность (любой формат: 1, "1", True)
    ...
    # Если срок вышел, деактивируем автоматически
    if days < 0:
        activate_user_in_db(callback.from_user.id, active=0)
        await callback.message.edit_text("⚠️ <b>Срок вашей подписки истек.</b>", reply_markup=main_markup(), parse_mode="HTML")
        return
...
async def show_tariffs(callback: CallbackQuery):
    d = get_user_db_data(callback.from_user.id)
    # ЗАПРЕТ ПОКУПКИ ПРИ АКТИВНОЙ ПОДПИСКЕ
    if d and str(d[4]) in ["1", "True"]:
        await callback.message.edit_text("✅ <b>У вас уже есть активная подписка!</b>\nПовторная покупка будет доступна после завершения текущего срока.", reply_markup=main_markup(), parse_mode="HTML")
        return
...
async def show_guide(callback: CallbackQuery):
    markup = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="to_main")]])
    await callback.message.edit_text("📖 <b>Инструкция:</b>\n1. Скачай Hiddify.\n2. Скопируй ссылку.\n3. Нажми 'Add from clipboard'.", reply_markup=markup, parse_mode="HTML")
...
async def admin_give(message: types.Message, command: CommandObject):
    ...
    if r:
        ...
        if lnk:
            activate_user_in_db(r[0], active=1); await bot.send_message(r[0], f"🎁 Доступ выдан!\n{hcode(lnk)}")
