...
# --- БЛОК БАЗЫ ДАННЫХ (БЕЗ ИЗМЕНЕНИЙ) ---
def init_db():
...
async def process_buy(callback: CallbackQuery):
    ...
    m = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 Оплатить на Lava.top", url=url)],
    ...
    await callback.message.edit_text(f"Вы выбрали <b>{plan_map.get(plan_key)}</b>. После оплаты нажмите кнопку ниже.", reply_markup=m, parse_mode="HTML")
...
async def user_paid(callback: CallbackQuery):
    ...
    adm_m = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Одобрить и выдать код", callback_data=f"adm_ap_{d[1]}_{plan_key}")],
        [InlineKeyboardButton(text="❌ Отклонить", callback_data=f"adm_dec_{d[1]}")]
    ])
    for a in ADMINS:
        if a != 0:
            try: await bot.send_message(a, f"💰 Заявка на {plan_map.get(plan_key)} от @{callback.from_user.username}", reply_markup=adm_m)
            except: pass
    await callback.answer("⏳ Сообщение отправлено админам!", show_alert=True)
...
async def adm_ap(callback: CallbackQuery):
    ...
    # Активация и генерация ключа
    expiry_ts = await activate_user_in_db(uid, plan=plan_name)
    vpn_code = await asyncio.get_event_loop().run_in_executor(None, get_vpn_link, uid, expiry_ts, plan_name)
    
    try:
        await bot.send_message(uid, f"✅ <b>Оплата подтверждена!</b>\n\nВаш ключ {plan_name}:\n{hcode(vpn_code)}", parse_mode="HTML")
    except: pass
    await callback.message.edit_text(f"✅ Готово. Ключ отправлен пользователю {uid}")

@router.callback_query(F.data.startswith("adm_dec_"))
async def adm_dec(callback: CallbackQuery):
    uid = int(callback.data.split("_")[2])
    try: await bot.send_message(uid, "❌ Ваш платеж не подтвержден.")
    except: pass
...
async def to_main(callback: CallbackQuery):
    await callback.message.edit_text("🚀 Главное меню:", reply_markup=main_panel())
...
async def show_ref(callback: CallbackQuery):
    ...
    await callback.message.edit_text(f"🤝 Приглашено: {d[5] if d else 0}/5\n\nСсылка:\n{hcode(link)}", reply_markup=back_btn(), parse_mode="HTML")
...
async def about_menu(callback: CallbackQuery):
    btns = [[InlineKeyboardButton(text="🆘 Поддержка", url=f"https://t.me/{SUPPORT_CONTACT.replace('@','')}")],
            [InlineKeyboardButton(text="⬅️ Назад", callback_data="to_main")]]
    await callback.message.edit_text("📖 TrubaVPN", reply_markup=InlineKeyboardMarkup(inline_keyboard=btns))
