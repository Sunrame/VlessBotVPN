...
def get_user_stats(user_id, username):
    ...
    try:
        ...
        if stats and sett:
            return {"used": stats.get('up', 0) + stats.get('down', 0), "limit": sett.get('totalGB', 0), "online": active_ips}
    except: pass
    return None
...
async def cmd_start(message: types.Message, command: CommandObject):
    ...
    await message.answer(f"👋 Привет, {hbold(message.from_user.full_name)}! Выбери действие:", reply_markup=main_markup(), parse_mode="HTML")
...
async def show_tariffs(callback: CallbackQuery):
    ...
    markup = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💳 Оплатить 250₽", url=url)],
...
async def show_ref(callback: CallbackQuery):
    ...
    me = await bot.get_me()
    markup = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="to_main")]])
    await callback.message.edit_text(f"🤝 <b>Партнерка</b>\nПригласи 5 друзей (купивших подписку) и получи месяц!\n\n📈 Прогресс: <b>{d[1]}/5</b>\n🔗 Ссылка:\n{hcode(f'https://t.me/{me.username}?start={callback.from_user.id}')}", reply_markup=markup, parse_mode="HTML")

@router.callback_query(F.data == "guide")
async def show_guide(callback: CallbackQuery):
    markup = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⬅️ Назад", callback_data="to_main")]])
    text = ("📖 <b>Инструкция по подключению:</b>\n\n"
            "1. Скачай приложение <b>Hiddify Next</b> или <b>v2rayNG</b>.\n"
            "2. Скопируй ссылку, которую выдал бот.\n"
            "3. В приложении выбери 'Add from clipboard' (Добавить из буфера).\n"
...
async def adm_ap(callback: CallbackQuery):
    ...
    if lnk:
        ...
        if u_data and u_data[0]:
            conn = sqlite3.connect('users.db'); conn.execute('UPDATE users SET bought_friends = bought_friends + 1 WHERE user_id = ?', (u_data[0],)); conn.commit(); conn.close()
...
async def admin_give(message: types.Message, command: CommandObject):
    ...
    if r:
        ...
        if lnk:
            activate_user_in_db(r[0], active=1); await bot.send_message(r[0], f"🎁 Бонус!\n{hcode(lnk)}")
...
async def admin_take(message: types.Message, command: CommandObject):
    ...
    if r:
        ...
        activate_user_in_db(r[0], active=0); await bot.send_message(r[0], "⚠️ Подписка аннулирована.")
