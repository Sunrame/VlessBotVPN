        ...
        payload = {
            ...
            "settings": json.dumps({
                "clients": [{
                    ...
                    "totalGB": limit, "expiryTime": exp, "enable": True, "subId": u_uuid
                }]
            })
        }
...
async def show_tariffs(callback: CallbackQuery):
    ...
    if d and str(d[4]) in ["1", "True"]:
        await callback.message.edit_text("✅ <b>Подписка уже активна!</b>", reply_markup=main_markup(), parse_mode="HTML")
        return
...
async def show_guide(callback: CallbackQuery):
    text = ("📖 <b>Инструкция:</b>\n\n"
            "1. Скачай и установи приложение <b>Happ</b>.\n"
            "2. Скопируй ссылку из этого бота.\n"
            "3. В приложении нажми 'Добавить профиль' (из буфера).\n"
...
async def to_main(callback: CallbackQuery):
    await callback.message.edit_text("Выбери раздел:", reply_markup=main_markup())
...
async def admin_give(message: types.Message, command: CommandObject):
    ...
    if r:
        ...
        if lnk:
            ...
            await bot.send_message(r[0], f"🎁 <b>Вам выдан доступ!</b>\n\n{hcode(lnk)}", parse_mode="HTML")
